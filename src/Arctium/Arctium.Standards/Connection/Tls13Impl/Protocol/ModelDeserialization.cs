...
namespace Arctium.Standards.Connection.Tls13Impl.Protocol
{
    class ModelDeserialization
    {
        ...
        private Extension DeserializeExtension_KeyShare_Client(byte[] buf, int offs)
        {
            ...
            group = (NamedGroup)(buf[cursor + 0] << 8 | buf[cursor + 1] << 0);
            ...
            if (length == 2)
            {
                return new KeyShareHelloRetryRequestExtension(group);
            }
        ...
        private Extension DeserializeExtension_KeyShare_Server(byte[] buffer, int offset)
        {
            ...
            if (clientSharedLength > 0)
            {
                ...
                while (!cursor.OnMaxPosition)
                {
                    ...
                    NamedGroup group = (NamedGroup)MemMap.ToUShort2BytesBE(buffer, cursor);
                    cursor += 2;
        ...
        private Extension DeserializeExtension_SupportedGroups(byte[] buffer, int offset)
        {
            ...
            List<NamedGroup> curves = new List<NamedGroup>();
            ...
            while (cursor < cursor.MaxPosition)
            {
                ...
                NamedGroup namedCurve = (NamedGroup)MemMap.ToUShort2BytesBE(buffer, cursor);
