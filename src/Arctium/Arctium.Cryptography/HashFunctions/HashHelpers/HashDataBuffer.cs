...
namespace Arctium.Cryptography.HashFunctions
{
    ...
    class HashDataBuffer
    {
        ...
        Action<byte[],int,int> limitReachedCallback;
        int bufferSize;

        public HashDataBuffer(int bufferSize, Action<byte[],int,int> limitReachedCallback)
        {
        ...
        public int Load(byte[] buffer, int offset, int length)
        {
            int totalCopied = 0;
            ...
            {
                System.Buffer.BlockCopy(buffer, offset, Buffer, DataLength, length);
                DataLength += length;
            ...
            if (DataLength > 0)
            {
                System.Buffer.BlockCopy(buffer, offset,Buffer,DataLength, bufferSize - DataLength);
            ...
            int remaining = length - totalCopied;
            System.Buffer.BlockCopy(buffer, offset + totalCopied, Buffer, 0, remaining);
        ...
        public int Load(Stream inputStream)
        {
            ...
            do
            {
                readLength = inputStream.Read(Buffer, DataLength, bufferSize - DataLength);
