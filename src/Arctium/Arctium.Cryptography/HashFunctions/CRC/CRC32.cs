...
namespace Arctium.Cryptography.HashFunctions.CRC
{
    public class CRC32
    {
        ...
        private readonly bool inputReflected;
        private readonly bool resultReflected;
        private readonly uint initialValue;
        private readonly uint finalXorValue;
        ...
        {
            if (polynomial == DefaultPolynomial && false)
            {
                lookupTable = LookupTable;
            }
            else
            {
                uint[] customLookupTable = new uint[256];
                uint result = 0;
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    for (int j = 0; j < 8; j++)
                    {
                        ...
                        if ((num & 0x80000000) != 0)
                        {
                            ...
                            num = (num) ^ (polynomial >> 1);
                        }

                    }

                    customLookupTable[i] = result;
                    result = 0;
                }
        ...
        {
            uint reflectedUint = 0;

            for (int i = 0; i < 16; i++)
            {
                reflectedUint |= (uint)((value & (uint)((uint)1 << i)) << ((2 * (16 - i)) - 1));
                reflectedUint |= (uint)((value & (uint)((uint)0x80000000 >> i)) >> ((2 * (16 - i)) - 1));
            }
