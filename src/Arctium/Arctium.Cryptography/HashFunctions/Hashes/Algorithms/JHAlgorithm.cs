...
namespace Arctium.Cryptography.HashFunctions.Hashes.Algorithms
{
    public static class JHAlgorithm
    {
        ...
        public static void HashBytes(JHContext context, byte[] buffer, long offset, long length)
        {
            ulong[] input = new ulong[8];

            for (long i = offset; i < offset + length; i += InputBlockSizeInBytes)
            {
                MemMap.ToULong64BytesBE(buffer, i, input, 0);
                ExecuteCompressionFunction_F8(context, input);
            }

            context.HashedBytesCount += (ulong)length;
        }

        public static void HashLastBlock(JHContext context, byte[] buffer, long offset, long length)
        {
            ...
            ulong msgLenInBits = (context.HashedBytesCount + (ulong)length) * 8;
            ...
            if (isMsgLenMultiply512)
            {
                ...
                ExecuteCompressionFunction_F8(context, lastTwoBlocks);
            }
            else 
            {
        ...
        public static void ExecuteCompressionFunction_F8(JHContext context, ulong[] inputa)
        {
            ulong t0, t1;

            for (int i = 0; i < 8; i++) context.H[i] ^= inputa[i];
            ulong[] input = context.H;
            ...
            for (int i = 0; i < 42; i++)
            {
                ...
                Omega_Swap(ref input[2], ref input[3], 1 << (i % 7));
                Omega_Swap(ref input[6], ref input[7], 1 << (i % 7));
                Omega_Swap(ref input[10], ref input[11], 1 << (i % 7));
                Omega_Swap(ref input[14], ref input[15], 1 << (i % 7));
            }

            for (int i = 0; i < 8; i++) context.H[i + 8] ^= inputa[i];
        }

        static void Omega_Swap(ref ulong a1, ref ulong a2, int n)
        {
            if(n == 64)
            {
                ulong t = a2;
