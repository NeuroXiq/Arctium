...
namespace  Arctium.Cryptography.HashFunctions.Hashes
{
    static class SHA2_384_512_Shared
    {
        ...
        {
            int lastBlockLength = (int)(messageLengthInBytes % 128);

            //message length in bits can exceed long value,
            //after 8 bits left shift need to split to some 128-bit value representing length.
            ulong[] messageLengthInBits = new ulong[2];

            //0x0e == 0000 0111
            ulong first3Bits = (ulong)0x0e << 61;

            //upper bits
            messageLengthInBits[0] = ((ulong)messageLengthInBytes & first3Bits) >> 61;
            //lower bits
            messageLengthInBits[1] = (ulong)messageLengthInBytes << 3;

            int paddingLengthInBytes = 128 - lastBlockLength;
            //16-bytes length value + '0x80' byte not fit, add new block
            if (paddingLengthInBytes < 17) paddingLengthInBytes += 128;

            byte[] padding = new byte[paddingLengthInBytes];

            padding[0] = 0x80;
            BinConverter.ToBytesBE(padding, padding.Length - 16, messageLengthInBits[0]);
            BinConverter.ToBytesBE(padding, padding.Length - 8, messageLengthInBits[1]);
        ...
        static class BitLogic
        {
            ...
            public static ulong BSIG0(ulong x)
            {
                return ROTR(x, 28) ^ ROTR(x, 34) ^ ROTR(x, 39);
            }

            public static ulong BSIG1(ulong x)
            {
                return ROTR(x, 14) ^ ROTR(x, 18) ^ ROTR(x, 41);
            }

            public static ulong SSIG0(ulong x)
            {
                return ROTR(x, 1) ^ ROTR(x, 8) ^ (x >> 7);
            }

            public static ulong SSIG1(ulong x)
            {
                return ROTR(x, 19) ^ ROTR(x, 61) ^ (x >> 6);
            }


            public static ulong ROTR(ulong v, int n)
            {
                return (v >> n) | (v << (64 - n));
            }
        }
