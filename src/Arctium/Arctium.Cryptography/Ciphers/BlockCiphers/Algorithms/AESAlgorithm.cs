...
namespace Arctium.Cryptography.Ciphers.BlockCiphers.Algorithms
{
    public static unsafe class AESAlgorithm
    {
        ...
        // TEST CODE
        public static void EncryptBlock128(Context context, byte* input, long inOffset, byte* output, long outOffset)
        {
            byte* state = stackalloc byte[16];
            uint k;
            ...
            for (int i = 1; i < 10; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    // b4 = sbox[b4];
                    // 
                    // byte r0 = (byte)(GFMul(0x02, b1) ^ GFMul(0x03, b2) ^ b3 ^ b4);
                    // byte r1 = (byte)(b1 ^ GFMul(0x02, b2) ^ GFMul(0x03, b3) ^ b4);
                    // byte r2 = (byte)(b1 ^ b2 ^ GFMul(0x02, b3) ^ GFMul(0x03, b4));
                    // byte r3 = (byte)(GFMul(0x03, b1) ^ b2 ^ b3 ^ GFMul(0x02, b4));
                    ...
                    // state[p1] = (byte)((byte)((k >> 24) & 0xFF) ^ r0);
                    // state[p2] = (byte)((byte)((k >> 16) & 0xFF) ^ r1);
                    // state[p3] = (byte)((byte)((k >> 08) & 0xFF) ^ r2);
                    // state[p4] = (byte)((byte)((k >> 00) & 0xFF) ^ r3);
                }
            }

            SubBytes(state);
            ShiftRows(state); ShiftRows(state);
            AddRoundKey(state, context.ExpandedKey, 10);
        ...
        public static void DecryptSingleBlock128(Context context, byte* input, long inOffset, byte* output, long outOffset, int rounds)
        {
            byte* state = stackalloc byte[16];
            ...
            for (int i = 0; i < 4; i++)
            {
                uint k = context.ExpandedKey[4 * 10 + i];

                state[(0 * 4) + i] ^= (byte)(k >> 24);
                state[(1 * 4) + i] ^= (byte)(k >> 16);
                state[(2 * 4) + i] ^= (byte)(k >> 08);
                state[(3 * 4) + i] ^= (byte)(k >> 00);
            }
            ...
            for (int i = rounds - 1; i >= 1; i--)
            {
                for (int k = 0; k < 4; k++)
                {
                    ...
                    uint key = context.ExpandedKey[k + (i * 4)];
                    // col ^= key;
            ...
            for (int k = 0; k < 4; k++)
            {
                ...
                uint key = context.ExpandedKey[k + (0 * 4)];
                // col ^= key;
        ...
        public static readonly uint[] InvT4 = new uint[]
{
        ...
        public static void GENERATE_CODE_DECRYPT_ROUND(int ROUNDS)
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                ...
                Console.WriteLine($"output[{p1}] = b1;");
                Console.WriteLine($"output[{p2}] = b2;");
                Console.WriteLine($"output[{p3}] = b3;");
                Console.WriteLine($"output[{p4}] = b4;");
            }
        }
