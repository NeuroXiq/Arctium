...
namespace Arctium.Protocol.Tls13Impl.Protocol
{
    internal class Crypto
    {
        ...
        private Tls13.Tls13ServerConfig config;

        public Crypto(Endpoint currentEndpoint, Tls13.Tls13ServerConfig config)
        {
            this.config = config;
        ...
        public void SetupCryptoAlgorithms(CipherSuite suite)
        {
            ...
            hkdf = new HKDF(new HMAC(hkdfHashFunc, new byte[0], 0, 0));
            selectedCipherSuite = suite;
        }
        ...
        public void ComputeSharedSecret(SupportedGroupExtension.NamedGroup namedGroup, byte[] privateKey, byte[] keyExchangeRawBytes)
        {
            if (namedGroup == SupportedGroupExtension.NamedGroup.Xx448)
            {
                Ecdhe_or_dhe_SharedSecret = RFC7748.X448(privateKey, keyExchangeRawBytes);
            }
            else if (namedGroup == SupportedGroupExtension.NamedGroup.X25519)
            {
                byte[] sharedSecret = RFC7748.X25519(privateKey, keyExchangeRawBytes);

                Ecdhe_or_dhe_SharedSecret = sharedSecret;
            }
            else if ((new[] { SupportedGroupExtension.NamedGroup.Secp256r1,
                    SupportedGroupExtension.NamedGroup.Secp384r1,
                    SupportedGroupExtension.NamedGroup.Secp521r1 })
                        .Any(x => x == namedGroup))
            {
                ...
                Ecdhe_or_dhe_SharedSecret = SEC1_ECFpAlgorithm.EllipticCurveDiffieHellmanPrimitive(ecparams, privateKey, clientPoint);
            }
            else if (
                namedGroup == SupportedGroupExtension.NamedGroup.Ffdhe2048 ||
                namedGroup == SupportedGroupExtension.NamedGroup.Ffdhe3072 ||
                ...
                namedGroup == SupportedGroupExtension.NamedGroup.Ffdhe8192)
            {
                ...
                Ecdhe_or_dhe_SharedSecret = FFDHE.ComputeSharedSecret(ffdheParams, privateKey, keyExchangeRawBytes);
            }
            else throw new NotSupportedException();
        }
        ...
        {
            SetupCryptoAlgorithms(serverHelloSuite);
        }
        ...
        public void SelectEcEcdheGroup(ClientHello hello, out bool groupOk)
        {
            ...
            if (groupOk) SelectedNamedGroup = supportedGroups[selectedGroupIdx];
        }
        ...
        public void SetupEarlySecret(byte[] psk)
        {
            ...
            EarlySecret = new byte[hashFunction.HashSizeBytes];
        ...
        public void SetupHandshakeSecret(ByteBuffer hscontext)
        {
            ...
            hkdf.Extract(derived, Ecdhe_or_dhe_SharedSecret, HandshakeSecret);
        ...
        public void SetupMasterSecret(ByteBuffer hscontext)
        {
            ...
            byte[] zeroValueOfHashLen = new byte[hashFunction.HashSizeBytes];
            MasterSecret = new byte[hashFunction.HashSizeBytes];
            ...
            byte[] clientBinderValue)
        {
            ...
            var baseKey = GenerateBinderKey(psk, hkdf, hashFunction);
        ...
        public byte[] ComputeBinderValue(ByteBuffer hscontextToBinders, Tls13.PskTicket ticket)
        {
        ...
        {
            return HkdfExpandLabel(hkdf, secret, labelText, context, length);
        }
