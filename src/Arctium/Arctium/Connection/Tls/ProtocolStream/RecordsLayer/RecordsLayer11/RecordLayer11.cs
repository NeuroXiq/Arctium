...
namespace Arctium.Connection.Tls.ProtocolStream.RecordsLayer.RecordsLayer11
{
    ...
    class RecordLayer11
    {
        ...
        private RecordLayer11(RecordIO recordIO, SecParams11 secParams)
        {
        ...
        public static RecordLayer11 Initialize(RecordIO recordIO)
        {
            SecParams11 initSecParams = InitSecParams();
            RecordLayer11 recordLayer = new  RecordLayer11(recordIO, initSecParams);
        ...
        public void Write(byte[] buffer, int offset, int count, ContentType type)
        {
            ...
            for (int i = 0; i < buffers.Length; i++)
            {
                byte[] transformedFragment = 
                    tlsCiphertextTransform.ForwardTransform(buffers[i], 0, buffers[i].Length, recordIO.WriteCount - 1);
        ...
        private void LoadFragmentToFragmentsStream()
        {
            ...
            tlsCiphertextTransform.ReverseTransform(tempBuf, 5, recordLength - 5, recordIO.ReadCount - 1);
