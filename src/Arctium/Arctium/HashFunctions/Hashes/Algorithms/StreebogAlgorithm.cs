...
{
    public unsafe class StreebogAlgorithm
    {
        ...
        {
            byte hStateInitial = state.HashSize == HashSize.Size512 ? (byte)0 : (byte)1;

            MemOps.Memset(state.h, 0, state.h.Length, hStateInitial);
            MemOps.Memset(state.Epsilon, 0, state.Epsilon.Length, 0);
            state.N = 0;
        }
        ...
        {
            byte* hm = stackalloc byte[64];
            byte* Ki = stackalloc byte[64];
            byte* accu = stackalloc byte[64];

            for (int i = 0; i < 8; i++) hm[63 - i] = ((byte)(state.N >> (i * 8)));
            for (int i = 0; i < 64; i++) hm[i] = (byte)(hm[i] ^ state.h[i]);
            LPS(hm);

            /* 'E' function start */

            MemCpy.Copy(hm, 0, Ki, 0, 64);
            MemCpy.Copy(input, inputOffset, accu, 0, 64);

            for (int i = 0; i < 12; i++)
            {
                for (int j = 0; j < 64; j++) accu[j] = (byte)(accu[j] ^ Ki[j]);

                LPS(accu);

                for (int j = 0; j < 64; j++) Ki[j] = (byte)(Ki[j] ^ Ci[i][j]);

                LPS(Ki);
            }

            for (int j = 0; j < 64; j++) accu[j] = (byte)(accu[j] ^ Ki[j]);
            MemCpy.Copy(accu, 0, hm, 0, 64);

            /* 'E' function end */

            for (int i = 0; i < 64; i++) hm[i] = (byte)(hm[i] ^ input[i] ^ state.h[i]);
            MemCpy.Copy(hm, 0, state.h, 0, 64);
        }

        static void LPS(byte* buffer)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                temp[i] =
                    LSMatrix[(256 * 0) + (uint)buffer[Tau[(8 * i) + 0]]] ^
                    LSMatrix[(256 * 1) + (uint)buffer[Tau[(8 * i) + 1]]] ^
                    LSMatrix[(256 * 2) + (uint)buffer[Tau[(8 * i) + 2]]] ^
                    LSMatrix[(256 * 3) + (uint)buffer[Tau[(8 * i) + 3]]] ^
                    LSMatrix[(256 * 4) + (uint)buffer[Tau[(8 * i) + 4]]] ^
                    LSMatrix[(256 * 5) + (uint)buffer[Tau[(8 * i) + 5]]] ^
                    LSMatrix[(256 * 6) + (uint)buffer[Tau[(8 * i) + 6]]] ^
                    LSMatrix[(256 * 7) + (uint)buffer[Tau[(8 * i) + 7]]];
            }

            MemMap.ToBytes8ULongBE(temp, 0, buffer, 0);
        }

        static void AddToEpsilon(State state, byte[] input, long inputOffset)
        {
            uint* toadd = stackalloc uint[16];

            MemMap.ToUInt64BytesBE(input, inputOffset, toadd, 0);

            uint carry = 0;

            for (int i = 15; i >= 0; i--)
            {
                ulong q = (ulong)toadd[i];
                ulong res = (ulong)state.Epsilon[i] + q + (ulong)carry;

                if (res > (ulong)0x00000000FFFFFFFF) carry = 1;
                else carry = 0;

                state.Epsilon[i] = (uint)res;
            }
        ...
        {
            byte[] lastBlock, nBytes, epsilonBytes;
            ...
            g(state, lastBlock, 0);
            state.N += (ulong)(length * 8);
            AddToEpsilon(state, lastBlock, 0);

            MemMap.ToBytes1ULongBE(state.N, nBytes, 56);
            MemMap.ToBytes16UIntBE(state.Epsilon, 0, epsilonBytes, 0);

            state.N = 0;
            g(state, nBytes, 0);
            g(state, epsilonBytes, 0);
        }

        static StreebogAlgorithm()
        {
            Ci = new byte[][] { C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12 };
        }

   static ulong[] LMatrix = new ulong[] { 
   ...
   0x07e095624504536c,  0x8d70c431ac02a736,  0xc83862965601dd1b,  0x641c314b2b8ee083, 
    ...
    static byte[] Tau = new byte[] {
        ...
        static readonly byte[][] Ci;

  static byte[] Pi = new byte[] { 252, 238, 221,  17, 207, 110,  49,  22, 251, 196, 250,
...
0x37, 0x8e, 0xe7, 0x67, 0xf1, 0x16, 0x31, 0xba, 0xd2, 0x13, 0x80, 0xb0, 0x04, 0x49, 0xb1, 0x7a, 
        ...
        static readonly ulong[] LSMatrix = new ulong[]
        {
           0xE63F55CE97C331D0,
0x25B506B0015BBA16,
0xC8706E29E6AD9BA8,
...
0x27945A985D5BD4D6,
        ...
        static void S(byte[] buffer)
        {
            for (int i = 0; i < 64; i++) buffer[i] = Pi[buffer[i]];
        }

        static void L(byte[] buffer)
        {
            ulong[] a = new ulong[8];
            MemMap.ToULong64BytesBE(buffer, 0, a, 0);
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < 64; j++)
                {
                    ulong r = (ulong)(a[i] & (ulong)((ulong)1 << (63 - j)));
                    if (r != 0)
                    {
                        result ^= LMatrix[j];
                    }
                ...
                MemMap.ToBytes1ULongBE(result, buffer, (i * 8));
            }
        }

        /// <summary>
        /// Code used to generate matrix
        /// </summary>
        public static void GeneratePSLookupMatrix()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 256; j++)
                {
                    byte inputByte = (byte)j;
                    int shift = 8 * i;

                    byte afterpi = Pi[inputByte];
                    ulong result = 0;
                    for (int k = 0; k < 8; k++)
                    {
                        if ((afterpi & (1 << k)) != 0) result ^= LMatrix[(8 * i) + 7 - k];
                    }

                    matrix.Add("0x" + result.ToString("X8") + ",");
                }
            }

            System.IO.File.WriteAllLines("C:\\my-files\\projects\\matrix.txt", matrix);
        }
        ...
        static void P(byte[] buffer)
        {
            ...
            for (int i = 0; i < 64; i++) result[i] = buffer[Tau[i]];

            MemCpy.Copy(result, 0, buffer, 0, 64);
        }

        public static void EFunctionExpanded()
        {
            byte[][] Ci = new byte[][] { C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12 };
            byte* Ki = stackalloc byte[64];
            byte* accu = stackalloc byte[64];
            // MemCpy.Copy(hm, 0, Ki, 0, 64);
            // MemCpy.Copy(input, inputOffset, accu, 0, 64);
            ...
            for (int i = 0; i < 12; i++)
            {
                for (int j = 0; j < 64; j++) Console.WriteLine($"accu[{j}] = (byte)(accu[{j}] ^ Ki[{j}]);");

                Console.WriteLine("LPS(accu);");

                for (int j = 0; j < 64; j++) Console.WriteLine($"Ki[{j}] = (byte)(Ki[{j}] ^ (0x{Ci[i][j].ToString("X16")});");

                Console.WriteLine("LPS(Ki);");
            }
        }

        /// <summary>
        /// 'E' function as streebog rfc doc specifies.
        /// </summary>
        /// <param name="K"></param>
        /// <param name="m"></param>
        static void E(byte* K, byte[] m)
        {
            byte[][] Ci = new byte[][] { C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12 };
            byte* Ki = stackalloc byte[64];
            byte* accu = stackalloc byte[64];
            MemCpy.Copy(K, 0, Ki, 0, 64);
            MemCpy.Copy(m, 0, accu, 0, 64);
