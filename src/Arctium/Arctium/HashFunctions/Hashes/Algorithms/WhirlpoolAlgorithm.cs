...
namespace Arctium.Cryptography.HashFunctions.Hashes.Algorithms
{
    public static class WhirlpoolAlgorithm
    {
        ...
        public class State
        {
            public ulong[] H;
            public ulong[] Hnext;
            public ulong[] ULongInput;
            public ulong[] KeySchedule;
            public ulong[] KeyScheduleRoundConst;
            public ulong[] TempOut;
        ...
        public static State InitState()
        {
            ...
            state.H = new ulong[8];
            state.Hnext = new ulong[8];
            state.ULongInput = new ulong[8];
            state.KeyScheduleRoundConst = new ulong[8];
            state.KeySchedule = new ulong[8 * 11];
            state.TempOut = new ulong[8];
        ...
        {
            MemMap.ToBytes8ULongBE(state.H, 0, outputBuffer, outputOffset);
        }
        ...
        private static void ProcessBlock(State state, byte[] buffer, long offset)
        {
            ...
            MemMap.ToULong64BytesBE(buffer, offset, ulongInput, 0);

            W(state, state.H, ulongInput, 0, state.Hnext);

            for (int i = 0; i < 8; i++) state.Hnext[i] ^= (ulong)(state.H[i] ^ ulongInput[i]);
        ...
        private static void W(State state, ulong[] K, ulong[] input, int inputOffset, ulong[] output)
        {
            ulong[] keySchedule = state.KeySchedule;
            ulong[] roundConst = state.KeyScheduleRoundConst;

            MemCpy.Copy(K, 0, 8, keySchedule, 0);

            for (int i = 0; i < 10; i++)
            {
                roundConst[0] = KRoundConst[i];
                RoundFunc(roundConst, 0, keySchedule, (i) * 8, keySchedule, (i + 1) * 8);
            }

            ulong[] tempOut = state.TempOut;

            for (int i = 0; i < 8; i++) tempOut[i] = (keySchedule[i] ^ input[inputOffset + i]);

            for (int i = 0; i < 10; i++)
            {
                RoundFunc(keySchedule, (i+1) * 8, tempOut, 0, output, 0);
                MemCpy.Copy(output, 0, 8, tempOut, 0);
            }

        }

        static void RoundFunc(ulong[] key, int keyOffset, ulong[] input, int inputOffset, ulong[] output, int outputOffset)
        {
            int o = outputOffset;
            int io = inputOffset;
            ulong[] inp = input;
            ...
            output[o + 0] = 
                Row0[(inp[io + 0] >> 56) & 0xFF] ^
                Row1[(inp[io + 7] >> 48) & 0xFF] ^
                Row2[(inp[io + 6] >> 40) & 0xFF] ^
                Row3[(inp[io + 5] >> 32) & 0xFF] ^
                Row4[(inp[io + 4] >> 24) & 0xFF] ^
                Row5[(inp[io + 3] >> 16) & 0xFF] ^
                Row6[(inp[io + 2] >> 08) & 0xFF] ^
                Row7[(inp[io + 1] >> 00) & 0xFF];

            output[o + 1] = 
                Row0[(inp[io + 1] >> 56) & 0xFF] ^
                Row1[(inp[io + 0] >> 48) & 0xFF] ^
                Row2[(inp[io + 7] >> 40) & 0xFF] ^
                Row3[(inp[io + 6] >> 32) & 0xFF] ^
                Row4[(inp[io + 5] >> 24) & 0xFF] ^
                Row5[(inp[io + 4] >> 16) & 0xFF] ^
                Row6[(inp[io + 3] >> 08) & 0xFF] ^
                Row7[(inp[io + 2] >> 00) & 0xFF];


            output[o + 2] = 
                Row0[(inp[io + 2] >> 56) & 0xFF] ^
                Row1[(inp[io + 1] >> 48) & 0xFF] ^
                Row2[(inp[io + 0] >> 40) & 0xFF] ^
                Row3[(inp[io + 7] >> 32) & 0xFF] ^
                Row4[(inp[io + 6] >> 24) & 0xFF] ^
                Row5[(inp[io + 5] >> 16) & 0xFF] ^
                Row6[(inp[io + 4] >> 08) & 0xFF] ^
                Row7[(inp[io + 3] >> 00) & 0xFF];

            output[o + 3] = 
                Row0[(inp[io + 3] >> 56) & 0xFF] ^
                Row1[(inp[io + 2] >> 48) & 0xFF] ^
                Row2[(inp[io + 1] >> 40) & 0xFF] ^
                Row3[(inp[io + 0] >> 32) & 0xFF] ^
                Row4[(inp[io + 7] >> 24) & 0xFF] ^
                Row5[(inp[io + 6] >> 16) & 0xFF] ^
                Row6[(inp[io + 5] >> 08) & 0xFF] ^
                Row7[(inp[io + 4] >> 00) & 0xFF];

            output[o + 4] = 
                Row0[(inp[io + 4] >> 56) & 0xFF] ^
                Row1[(inp[io + 3] >> 48) & 0xFF] ^
                Row2[(inp[io + 2] >> 40) & 0xFF] ^
                Row3[(inp[io + 1] >> 32) & 0xFF] ^
                Row4[(inp[io + 0] >> 24) & 0xFF] ^
                Row5[(inp[io + 7] >> 16) & 0xFF] ^
                Row6[(inp[io + 6] >> 08) & 0xFF] ^
                Row7[(inp[io + 5] >> 00) & 0xFF];

            output[o + 5] = 
                Row0[(inp[io + 5] >> 56) & 0xFF] ^
                Row1[(inp[io + 4] >> 48) & 0xFF] ^
                Row2[(inp[io + 3] >> 40) & 0xFF] ^
                Row3[(inp[io + 2] >> 32) & 0xFF] ^
                Row4[(inp[io + 1] >> 24) & 0xFF] ^
                Row5[(inp[io + 0] >> 16) & 0xFF] ^
                Row6[(inp[io + 7] >> 08) & 0xFF] ^
                Row7[(inp[io + 6] >> 00) & 0xFF];

            output[o + 6] = 
                Row0[(inp[io + 6] >> 56) & 0xFF] ^
                Row1[(inp[io + 5] >> 48) & 0xFF] ^
                Row2[(inp[io + 4] >> 40) & 0xFF] ^
                Row3[(inp[io + 3] >> 32) & 0xFF] ^
                Row4[(inp[io + 2] >> 24) & 0xFF] ^
                Row5[(inp[io + 1] >> 16) & 0xFF] ^
                Row6[(inp[io + 0] >> 08) & 0xFF] ^
                Row7[(inp[io + 7] >> 00) & 0xFF];

            output[o + 7] = 
                Row0[(inp[io + 7] >> 56) & 0xFF] ^
                Row1[(inp[io + 6] >> 48) & 0xFF] ^
                Row2[(inp[io + 5] >> 40) & 0xFF] ^
                Row3[(inp[io + 4] >> 32) & 0xFF] ^
                Row4[(inp[io + 3] >> 24) & 0xFF] ^
                Row5[(inp[io + 2] >> 16) & 0xFF] ^
                Row6[(inp[io + 1] >> 08) & 0xFF] ^
                Row7[(inp[io + 0] >> 00) & 0xFF];

            for (int i = 0; i < 8; i++) output[o + i] ^= key[keyOffset + i];
        }

        // Theta function (multiply State row matrix by 'C' Matrix in GF field)
        ...
        // Code below used to generate Precomputed tables below
        // Tables represents all possible byte multiply by C row
        // matrix after S-box applied
        // Not used in the code, just to keep how this works
        // This is idea from specification to 'Compute row function with 7 xors and 8 table lookups'
        // similar like in reference code of Whirlpool

        static void Generate()
        {
            for (int b = 0; b < 8; b++)
            {
                System.Console.WriteLine("Row{0}", b);
            for (int i = 0; i < 256; i++)
            {
                byte s = S[i];
                    ...
                    System.Console.Write("0x");
                    {
                        ...
                        System.Console.Write("{0:X2}", mul);
                        }

                    }
                    System.Console.Write(", ");
                    if ((i+1) % 4 == 0) System.Console.WriteLine();
            }
            }
        }
...
static readonly ulong[] Row1 = new ulong[]
{
...
static readonly ulong[] Row2 = new ulong[]
{
...
static readonly ulong[] Row3 = new ulong[]
{
...
static readonly ulong[] Row4 = new ulong[]
{
...
static readonly ulong[] Row5 = new ulong[]
{
...
static readonly ulong[] Row6 = new ulong[]
{
...
static readonly ulong[] Row7 = new ulong[]
{
...
0x28A0285D88507528, 0x5C6D5CDA31B8865C, 0xF8C7F8933FED6BF8, 0x86228644A411C286,
        ...
        static readonly ulong[] KRoundConst = new ulong[]
        {
            0x1823c6e887b8014f,
            0x36a6d2f5796f9152,
            0x60bc9b8ea30c7b35,
