...
{
    public static unsafe class RadioGatun64Algorithm
    {
        ...
        {
            ulong* input = stackalloc ulong[3];
            ulong* a = stackalloc ulong[19];
            ulong* b = stackalloc ulong[13 * 3];
            ulong* A = stackalloc ulong[19];
            ulong* B = stackalloc ulong[13 * 3];
            ...
            fixed (byte* pBuffer = &buffer[offset])
            {
                ...
                for (long j = 0; j < length; j += 24)
                {
                    MemMap.ToULong24BytesLE(pBuffer + j, input);
                ...
                MemCpy.Copy(a, state.a);
                MemCpy.Copy(b, state.b);
            }
        }
        ...
        // 
        [MethodImpl(MethodImplOptions.AggressiveOptimization | MethodImplOptions.AggressiveInlining)]
        static void R_Optimized(State state, ulong* a, ulong* b, ulong* A, ulong* B)
        {
            B[0] = b[36];
            B[1] = b[37];
            B[2] = b[38];
            ...
            a[8] = BinOps.ROR(A[18], 36);
            a[9] = BinOps.ROR(A[6], 45);
            a[10] = BinOps.ROR(A[13], 55);
            a[11] = BinOps.ROR(A[1], 2);
            a[12] = BinOps.ROR(A[8], 14);
            a[13] = BinOps.ROR(A[15], 27);
            a[14] = BinOps.ROR(A[3], 41);
            a[15] = BinOps.ROR(A[10], 56);
            a[16] = BinOps.ROR(A[17], 8);
            a[17] = BinOps.ROR(A[5], 25);
            a[18] = BinOps.ROR(A[12], 43);
            A[0] = a[0] ^ a[1] ^ a[4];
            A[1] = a[1] ^ a[2] ^ a[5];
            A[2] = a[2] ^ a[3] ^ a[6];
            A[3] = a[3] ^ a[4] ^ a[7];
        ...
        static void Generate()
        {
            ...
            for (int i = 0; i < 19; i++) Console.WriteLine($"a[{i}] = BinOps.ROR(A[{(7 * i) % 19}], {((i * (i + 1)) / 2) % 64});");
